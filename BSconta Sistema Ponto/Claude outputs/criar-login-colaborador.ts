...
//     servidor, um e-mail interno (só um identificador de login dentro do
//     Supabase Auth — não precisa existir como caixa de e-mail de verdade) e
//     uma senha aleatória, cria o login já confirmado (auth.admin.createUser
//     com email_confirm:true) e devolve as duas coisas na resposta, UMA
//     ÚNICA VEZ, para o RH que fez a chamada. Depois desta resposta, a senha
//     em texto puro não é guardada em lugar nenhum — se for perdida, é
...
// Domínio usado só para os e-mails de login interno gerados por esta função.
// Não precisa existir como caixa de e-mail real — é apenas o identificador
// dentro do Supabase Auth. Trocar aqui muda o padrão para os PRÓXIMOS logins
// gerados (não afeta os que já existem).
const DOMINIO_LOGIN_INTERNO = "colaboradores.bsconta.com.br";
...
/** Remove acentos, deixa minúsculo, troca espaços por ponto — "Ângela Dos
 * Santos" -> "angela.dos.santos". Usado só como base do e-mail interno. */
...
/** Senha aleatória de 14 caracteres, com letras (maiúsculas/minúsculas),
 * números e símbolos — gerada com crypto.getRandomValues (aleatoriedade
 * criptográfica real, não Math.random). */
...
Deno.serve(async (req) => {
  ...
  // ---------------------------------------------------------------------
  // 4) Monta o e-mail interno (só identificador de login, não precisa
  //    receber e-mail de verdade) e tenta criar — em caso de a base já
  //    existir (duas pessoas com nome igual, por exemplo), tenta de novo
  //    com um número no final, até 6 tentativas.
  // ---------------------------------------------------------------------
  const baseLocal = slugNome(nome) || "colaborador";
  ...
  for (let tentativa = 0; tentativa < 6; tentativa++) {
    const candidato = tentativa === 0 ? `${baseLocal}@${DOMINIO_LOGIN_INTERNO}` : `${baseLocal}${tentativa + 1}@${DOMINIO_LOGIN_INTERNO}`;
    const { data: criado, error: criarErr } = await adminClient.auth.admin.createUser({
      email: candidato,
      password: senha,
      email_confirm: true, // já confirmado — não existe caixa de e-mail real pra confirmar
